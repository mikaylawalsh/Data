...
# puzzles for correctness. This version of the project uses btest, which
...
my $tmpdir = "/var/tmp/dataproj.$login.$$";
...
# Make sure that an executable dpc (data project compiler) exists
(-e "./dpc" and -x "./dpc")
    or  die "$0: ERROR: No executable dpc binary.\n";
...
if ($USE_BTEST) {
    $driverfiles = "Makefile dpc btest.c decl.c tests.c btest.h bits.h";
    ...
    $driverfiles = "dpc tests.c bddcheck";
    unless (system("cp -r $driverfiles $tmpdir") == 0) {
	clean($tmpdir);
...
#
print "1. Running './dpc -z' to identify coding rules violations.\n";
system("cp bits.c save-bits.c") == 0
    or die "$0: ERROR: Could not create backup copy of bits.c. $diemsg\n";
system("./dpc -z -o zap-bits.c bits.c") == 0
...
#
# Run dpc to identify operator count violations.
# 
print "\n3. Running './dpc -Z' to identify operator count violations.\n";
system("./dpc -Z -o Zap-bits.c save-bits.c") == 0
    or die "$0: ERROR: dpc unable to generated Zapped bits.c file.\n";
...
#
# Run dpc to get the operator counts on the zapped input file
#
print "\n5. Running './dpc -e' to get operator count of each function.\n";
$status = system("./dpc -W1 -e zap-bits.c > dpc-opcount.out 2>&1");
if ($status != 0) {
...
#
# Collect the operator counts generated by dpc
#
open(INFILE, "$tmpdir/dpc-opcount.out") 
    or die "$0: ERROR: could not open input file $tmpdir/dpc-opcount.out\n";
